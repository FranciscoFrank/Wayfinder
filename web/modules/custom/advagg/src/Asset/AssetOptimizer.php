...
abstract class AssetOptimizer {
   ...
   *   The AdvAgg cache.
   */
  public function __construct(ConfigFactoryInterface $config_factory, EventDispatcherInterface $event_dispatcher, CacheBackendInterface $cache) {
  ...
  protected function writeFile($data, $cid) {
    ...
    if (!file_exists($uri)) {
      if (!$this->fileSystem->saveData($data, $uri, FileExists::Replace)) {
        return FALSE;
      }
    }
    ...
    if ($this->gZip && !file_exists($uri . '.gz')) {
      $this->fileSystem->saveData(gzencode($data, 9, FORCE_GZIP), $uri . '.gz', FileExists::Replace);
    }
    ...
    if ($this->brotli && !file_exists($uri . '.br')) {
      $this->fileSystem->saveData(brotli_compress($data, 11, BROTLI_TEXT), $uri . '.br', FileExists::Replace);
    }
    return $uri;
  }
...
EOT;
    $fileSystem->saveData($htaccess, $file, FileExists::Replace);
  }
